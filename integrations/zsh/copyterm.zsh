...
__copyterm_preexec() {
    ...
    [ -z "$cmd" ] && return
    local ts=$(date +%s%3N 2>/dev/null || date +%s)
    printf "\n\033]133;C;cmd=%s;ts=%s\007\n$ %s\n" "$cmd" "$ts" "$cmd" >> "$__copyterm_buf_file" 2>/dev/null
}
...
if [ -z "$__copyterm_bin" ] && [ -z "$__copyterm_py" ]; then
    _zsh_src="${(%):-%x}"
    if [ -n "$_zsh_src" ]; then
        _script_dir="$(cd "$(dirname "$_zsh_src")" 2>/dev/null && pwd)"
